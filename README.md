...
Lorsque les 2 bornes BTN sont reliées via un bouton poussoir, un ILS ou équivalent, le relais reste activé pour une temporisation réglable entre 1s et 3min 30s.

- **ACTIVATION** Reliez les 2 bornes (avec un bouton poussoir, ILS, ou équivalent) pour enclencher le relais pour un temps réglable de 1s à 3min 30s.
...
Installez kicad 6 (ou version supérieure), et ouvrez le fichier LEC011001.kicad_pro
...
```
cd hardware
docker run --rm -w /kikit -v $PWD:/kikit yaqwsx/kikit kikit fab jlcpcb --assembly --no-drc --schematic LEC011001.kicad_sch --field LCSC --corrections JLCPCB_CORRECTION --nametemplate LEC011001_{} LEC011001.kicad_pcb prod/
